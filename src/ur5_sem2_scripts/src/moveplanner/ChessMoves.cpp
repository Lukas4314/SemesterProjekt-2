...
{ // Adjust "ur_manipulator" as needed
...
bool ChessMoves::move(MoveStruct move, double TFchess[4][4])
{
    ...
    case 'm':
        if (move.captured != '-')
        {
            Logger::setValue(GRIPPER_PICKUP_ENEMY_PIECE, "1");
    ...
    case 'c':
        Logger::setValue(EXTRA_PICKUP_FOR_CASTLING, "1");
        return castle(move, TFchess);
    case 'e':
        return en_passant(move, TFchess);
    case 'k':
        Logger::setValue(GRIPPER_SHOULD_PICKUP_DEAD_PIECE, "1");
