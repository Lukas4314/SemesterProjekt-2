    ...
    */
    ...
    bool ChessBoard::applyIfValidMove(string move)
    {
        ...
        if (move == "wQ" && activeColor != "w" || move == "wK" && activeColor != "w" || (move == "bQ" && activeColor == "w") || (move == "bK" && activeColor == "w"))
        {
            RCLCPP_DEBUG(logger, "Nej tak til castling, det er ikke sigma");
            return false;
        }

        else if (move == "wQ" && activeColor == "w" || move == "wK" && activeColor == "w" || (move == "bQ" && activeColor != "w") || (move == "bK" && activeColor != "w"))
        {
            RCLCPP_DEBUG(logger, "Entered first if statement");
            bool castleBools[4];
            ...
            if (move == "wQ")
            {
                ...
                RCLCPP_DEBUG(logger, "activeColor is: %s", activeColor.c_str());
                activeColor = (activeColor == "w") ? "b" : "w";
                RCLCPP_DEBUG(logger, "activeColor is: %s", activeColor.c_str());
                wK = false;
                wQ = false;
        ...
        if (piece1 == '-')
        {
            ...
            string tempMove = move.substr(2,2) + move.substr(0,2);
            move = tempMove;
            RCLCPP_DEBUG(logger, "tempMove is: %s", move.c_str());
        }


        else if (activeColor == "w" && islower(piece1))
        {
            RCLCPP_DEBUG(logger, "Entered white move white piece");
            fromCol = move[0] - 'a';
            fromRow = 8 - (move[1] - '0');
        ...
        RCLCPP_DEBUG(logger, "Move validator siger hej");
