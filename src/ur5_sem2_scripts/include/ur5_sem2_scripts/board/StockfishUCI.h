...
public:
    ...
    StockfishUCI(const string &path = "/usr/games/stockfish")
    { // This constructer takes a path to the stockfish engine. If nothing is declared it defaults to the defined path
        if (pipe(toEngine) == -1 || pipe(fromEngine) == -1)
        {                                                  // Here we use the pipe function from the unistd.h class. Pipe allows us to create two new pipes which we are going to use to communicate with the stockfish engine.
        ...
        if (childPid == -1)
        {                                                  // So this is again just error checking
        ...
        if (childPid == 0)
        { // Checks the variable childPid which tells us wether or not it is the child or the parent program. In this case we choose/check for the child.
        ...
        { // This is ofcourse how we initiate the parent process
    ...
    {
        if (childPid > 0)
        {                                  // Makes sure we are on the parent (only the parent does the cleanup)
    ...
    string readOutputUntil(const string &token)
    {
        ...
        {                                                                    // Initiates while true loop
            ...
            {
                break;
            }                     // If we get less than 0 or nothing (so 0) from stockfish break this loop
            ...
            if (output.find(token) != string::npos)
                break; // Checks if the "token" so bestmove for instance is inside output. If it is we exit the loop. Later we can then read the best move
        }
        return output; // Function returns output
    }

    // Function for getting the best move.
    string getBestMove(const string &moves, int depth = 15)
    {                             // Sends two things. A string of moves (e2e4, a3a4 etc.) and an integer depth which is the amount of steps ahead stockfish should be
